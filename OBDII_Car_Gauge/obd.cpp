...
int probeOnePID(const char* cmd) {
  if (!BTSerial.connected()) return 2;
...
// ─────────────────────────────────────────────────────────────
static bool initELM() {
  if (!BTSerial.connected()) { Serial.println("OBD: BT not connected"); return false; }
...
static bool runBgProbe() {
  ...
  if (!bgProbeWaiting) {
    ...
    responseReady  = false;
    while (BTSerial.available()) BTSerial.read();
    BTSerial.print(String(STANDARD_PIDS[bgProbeIndex].cmd) + "\r");
